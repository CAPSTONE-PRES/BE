...
public class TokenService {
    ...
    public String createAccessToken(String refreshToken) {
        ...
        // 사용자 id로 액세스 토큰 생성 (설정값 사용)
        return tokenProvider.generateToken(user, jwtProperties.getAccessTokenDuration());
    }

        public String createTestAccessToken(String refreshToken) {
        // 토큰 유효성 검사에 실패하면 예외 발생
        if (!tokenProvider.validToken(refreshToken)) {
            throw new IllegalArgumentException("Invalid refresh token");
        }
        ...
        User user = userService.findById(userId);
        // 사용자 id로 액세스 토큰 생성 (설정값 사용)
        return tokenProvider.generateToken(user, jwtProperties.getTestAccessTokenDuration());
    }
