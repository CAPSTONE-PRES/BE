...
public class GenerateCueService {
    ...
    private String buildCueCardPrompt(String slideText, int slideNumber, int maxSections, int totalSlides) {
        return """
                ...
                - 발표 흐름이 자연스럽게 이어질 수 있도록 문장을 구성하되,
                  “다음으로 넘어가겠습니다”, “다음 내용은 ~입니다” 같은 불필요한 예고 문장은 넣지 않는다.
                  단, 슬라이드 자체에 번호 기반 소주제가 있을 경우에만
                  “다음은 ~입니다” 같은 구성 소개 문장을 사용할 수 있다.
                - 한 슬라이드 내에서 이미 언급된 내용을 반복하지 않는다.
                ...
                - 사용 규칙:
                  * <🌬 호흡>: 문단 시작 또는 긴 설명 전후 자연스럽게 배치
                  * <🔍 청중 바라보기>: 핵심 내용을 전달한 문장 뒤에 자연스럽게 배치
                  * <👉 화면 가리키기>: 시각적 자료(도표, 표, 주요 키워드 등)를 설명하는 문장 뒤에 **슬라이드당 한 번만** 사용
                    - 표지(1번) / 마지막 슬라이드에서는 기본적으로 사용하지 않는다.

                - 평균 길이가 긴 문단의 첫 문장에는 <🌬 호흡> 또는 <🔍 청중 바라보기>를 함께 사용할 수 있다.
                - 비언어 표현은 <> 를 반드시 지킨다.
                - 제스처(<✋>)는 사용하지 않는다.
                ...
                - 초과할 경우 상위 %d개만 포함하라.
                - 본문(text)에는 섹션 제목/번호/키워드/기호(#, [], 번호 등)를 포함하지 말라.
                ...
                [대본 작성 형식 참고]
                            ...
                            ⚠️ 주의:
                            - 위는 구조 참고용이며, 반드시 입력된 슬라이드의 실제 내용을 사용할 것.
                            - 예시 단어를 그대로 사용하지 말 것.

                ---

                [심화버전 작성 규칙]
                - advanced.sections 는 basic.sections 의 index를 그대로 사용한다. (개수 동일)
                - advanced.text 는 해당 basic.section의 keyword 및 text를 기반으로 한 문장 요약이어야 한다.
                - 슬라이드 전체 요약을 작성하지 말고, 각 섹션 내용을 한 문장씩 요약한다.
                - 자연스러운 설명문으로 작성하되 군더더기 없이 핵심만 포함한다.
                - 비언어적 표현 아이콘은 advanced에서 절대 사용하지 않는다.
                - 표지(1번 슬라이드)는 “이번 발표의 목적”만 간단히 설명한다.
                - OCR 인식 불가 시 “(OCR 인식 불가 – 요약 생략)”을 포함한다.

                ---

                [슬라이드 맥락 규칙]
                - 이 슬라이드는 전체 중 %d/%d 번째이다.
                - 표지(1번): 인사/주제소개 중심. 팀 소개는 표지에 있을 때만 가능.
                - 마지막 슬라이드(%d): 결론/감사/다음 단계 중심. 기능 설명 금지.

                ---

                [특수 슬라이드 유형 규칙]

                ※ 아래 규칙은 위의 일반 규칙보다 우선 적용된다.
                ※ 모델은 슬라이드 텍스트의 구조와 길이를 보고 해당 유형에 해당한다고 추론해야 한다.

                1) 표지 슬라이드(슬라이드 1번 또는 제목/부제/발표자 이름 정도만 있는 페이지로 추정되는 경우)
                - 인사 + 발표 주제 + 필요 시 팀/발표자 소개까지만 간단히 작성한다.
                - 기능 설명, 배경 설명, 세부 목차 설명은 절대 포함하지 않는다.
                - 문장은 2~3문장 이내로 유지한다.
                - 비언어 표현은 <🌬 호흡> 1회만 사용하며, <👉 화면 가리키기>는 사용하지 않는다.

                2) 목차 슬라이드(여러 개의 항목이 나열된 구조의 슬라이드로 추정되는 경우)
                - 각 항목을 자연스럽게 말로 풀어 소개하는 방식으로만 작성한다.
                - 각 항목의 세부 내용(정의, 예시, 배경 등)은 미리 설명하지 않는다.
                - 전체 3~4문장 이내에서 “어떤 흐름으로 발표가 진행되는지”만 안내한다.
                - 비언어 표현은 필요할 경우 <🌬 호흡>만 사용한다.

                3) 챕터 전환 슬라이드(“01. ~”, “2. 연구 방법” 등 큰 제목 위주로 구성된 슬라이드로 추정되는 경우)
                - 해당 챕터로 전환하는 한두 문장만 작성한다.
                - 본문 내용, 세부 설명, 예시를 추가로 작성하지 않는다.
                - basic.sections 는 최대 1개만 생성한다.
                - 비언어 표현은 <🌬 호흡>만 사용할 수 있다.

                4) 마지막 정리/감사 슬라이드(“결론”, “정리”, “Thank you”, “감사합니다” 등이 포함된 마지막 슬라이드로 추정되는 경우)
                - 핵심 요지 한두 줄 + 의의/느낀 점 한 줄 + 감사 인사 정도만 작성한다.
                - 새로운 정보, 세부 기능, 세부 데이터 설명은 추가하지 않는다.
                - 문장은 2~4문장 이내로 유지한다.
                - 비언어 표현은 <🌬 호흡> 1회만 사용하고, <👉 화면 가리키기>는 사용하지 않는다.
                ...
                - 각 section에는 index, keyword, text가 반드시 존재해야 한다.
                - advanced.text는 반드시 존재하며 비어있지 않아야 한다.
                - 반드시 입력된 슬라이드 텍스트의 실제 내용을 기반으로 작성한다.
                ...
                [금지사항]
                - JSON 외 텍스트 출력 금지
                - 예시 문장, 설명문, 마크다운, 서술문 출력 금지
                - slide 번호 오기입 금지 (반드시 %d)
                - 여러 슬라이드 내용 병합 금지
                - 섹션 누락 금지
                - advanced.text 누락 금지
                - 입력되지 않은 내용을 임의로 생성하는 행위 금지
                ...
                %s
                """.formatted(
                maxSections, // 1) basic index range (1~%d)
                maxSections, // 2) advanced index range (1~%d)
                slideNumber, // 3) "슬라이드 %d번"
                slideNumber, // 4) "slide = %d"
                slideNumber + 1, // 5) 예: %d
                slideNumber - 1, // 6) 예: %d
                maxSections, // 7) "최대 %d개"
                maxSections, // 8) "상위 %d개"
                slideNumber, // 9) "전체 중 %d/%d 번째"의 현재 번호
                totalSlides, // 10) "전체 중 %d/%d 번째"의 전체 개수
                totalSlides, // 11) "마지막 슬라이드(%d)"
